...
var (
	...
	//go:embed tests/pull_response_snapshot_hashes.bin
	testPullResponseSnapshotHashes []byte
	//go:embed tests/pull_response_version.bin
	testPullResponseVersion []byte
	//go:embed tests/pull_response_node_instance.bin
	testPullResponseNodeInstance []byte
)
...
func TestPullResponse_SnapshotHashes_Unmarshal(t *testing.T) {
	msg, err := UnmarshalMessage(testPullResponseSnapshotHashes)
	assert.NoError(t, err)
	assert.Equal(t, &PullResponse{
		Pubkey: [32]byte{
			0x7a, 0x1f, 0xe3, 0x85, 0x3f, 0x19, 0xa1, 0xcc, 0x52, 0x82, 0x2c, 0x0d, 0x03, 0x2d, 0x19, 0x16,
...
func TestPullResponse_Version_Unmarshal(t *testing.T) {
...
func TestPullResponse_NodeInstance_Unmarshal(t *testing.T) {
	msg, err := UnmarshalMessage(testPullResponseNodeInstance)
	assert.NoError(t, err)
	assert.Equal(t, &PullResponse{
		Pubkey: [32]byte{
			0x7a, 0x1f, 0xe3, 0x85, 0x3f, 0x19, 0xa1, 0xcc, 0x52, 0x82, 0x2c, 0x0d, 0x03, 0x2d, 0x19, 0x16,
